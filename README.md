...
| 학습 타겟 | [`Target` = **득점 / 만점**](https://github.com/gmrmsy/CLAP_D/blob/main/scr/data/split.py#L4-L14) — 0~1 사이의 연속 비율값 |

모델은 득점 비율(0~1)을 예측하고, [예측값에 해당 문항의 만점(`Score(Alloc)`)을 곱한 뒤 반올림](https://github.com/gmrmsy/CLAP_D/blob/main/scr/evaluate/test.py#L60-L62)하여 최종 정수 점수로 변환합니다.
...
| 구성 요소 | 세부 내용 | 코드 |
|-----------|-----------|------|
| 패딩 마스크 | 유효 음성 길이 추출, 패딩 제외 | [`SequenceMask`](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L41-L47) |
| CNN | Conv1D(512, kernel=11, stride=2) + Conv1D(512, kernel=11, stride=1) | [CNN 블록](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L66-L102) |
| 활성화 | HardTanh (clip [-20, 20]) | [hardtanh](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L8-L9) |
| RNN | Bidirectional GRU(64), dropout=0.1 | [GRU](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L107-L108) |
| 위치 인코딩 | Sinusoidal Positional Encoding | [`SinusoidalPositionalEncoding`](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L12-L39) |
| Attention | MultiHeadAttention(heads=16, key_dim=32, dropout=0.1) × 6 | [Attention 루프](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L122-L138) |
| 출력층 | Dense(1, activation=`out`) — relu 또는 linear | [출력 Dense](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L147-L151) |
| 학습 | Adam(lr=0.001), loss=MSE, EarlyStopping(patience=10) | [`model_train`](https://github.com/gmrmsy/CLAP_D/blob/main/scr/utils/train_utils.py#L4-L51) |
| 배치 크기 | 64, 최대 100 epoch | [`model.fit`](https://github.com/gmrmsy/CLAP_D/blob/main/scr/utils/train_utils.py#L42-L50) |
...
([`itertools.product`로 모든 조합 자동 생성](https://github.com/gmrmsy/CLAP_D/blob/main/scr/train/train.py#L37-L48))

| 변수 | 선택지 |
|------|--------|
| 학습 데이터 구성 | no1~no25, total, reliable |
| CNN 차원 | 1D / 2D |
| 출력 활성화 | relu / linear |
| 손실 가중치 | lossO (있음) / lossX (없음) |
| 데이터 증강 | aug 적용 / 미적용 |
...
| [`reliable`](https://github.com/gmrmsy/CLAP_D/blob/main/scr/utils/data_utils_class.py#L176-L179) | 점수 분산이 가장 높은 상위 3개 문항만 선택하여 학습 |
...
| 구분 | 방식 | 코드 |
|------|------|------|
| **1D** (baseline) | 멜 스펙트로그램을 `(시간, 주파수)` 형태로 변환 후 Conv1D 처리 | [1D CNN](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L68-L77) |
| **2D** | `(128, 312, 1)` 원본 형태 그대로 Conv2D 처리 후 Reshape | [2D CNN](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L79-L102) |
...
| 구분 | 출력층 활성화 | 의도 | 코드 |
|------|-------------|------|------|
| **relu** | `Dense(1, activation='relu')` | 예측값 ≥ 0 보장 | [출력 Dense](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L150-L151) |
| **linear** (baseline) | `Dense(1, activation='linear')` | 제약 없이 학습 | [출력 Dense](https://github.com/gmrmsy/CLAP_D/blob/main/scr/models/model_1D.py#L150-L151) |
...
### 4. 점수 분포 편향 보정: 손실 가중치 (lossO / lossX)

[`weight_return` 함수](https://github.com/gmrmsy/CLAP_D/blob/main/scr/utils/train_utils.py#L15-L37)에서 샘플 가중치를 계산하여 학습에 적용합니다.
...
| 증강 기법 | 구현 |
|-----------|------|
| 속도 변환 | [`speed_aug`](https://github.com/gmrmsy/CLAP_D/blob/main/scr/utils/augment_utils.py#L3-L25) — 멜 스펙트로그램 시간축 선형 보간 리샘플링 |
| 피치 이동 | [`pitch_aug`](https://github.com/gmrmsy/CLAP_D/blob/main/scr/utils/augment_utils.py#L27-L44) — 멜 빈(bin) 단위 주파수 축 이동 |
| 소수 샘플 선택 | [`augment` — idx_0/idx_1 분리](https://github.com/gmrmsy/CLAP_D/blob/main/scr/utils/data_utils_class.py#L119-L123) — Target≠1 샘플만 선택하여 증강 |

**의도**: 소수 샘플을 늘려 점수 분포 편향 완화  
**결과**: aug 적용 시 일부 문항에서 개선. 특히 1D_relu_lossX의 `no4_aug`가 corr=0.7001로 전체 최고 상관계수 달성.  
단, 이는 **특정 문항 + aug 조합에서의 이례적 현상**으로, total/reliable 예측에서는 개선되지 않아 신뢰하기 어려움.
...
| 지표 | 의미 | 구현 | 한계 |
|------|------|------|------|
| Accuracy | 예측값 == 정답인 비율 | [`np.mean(process_score == correct_y)`](https://github.com/gmrmsy/CLAP_D/blob/main/scr/evaluate/test.py#L69) | 점수 분포 편향 시 지배적 점수 예측만으로도 높게 나옴 |
| Pearson r | 예측값과 실제값의 선형 상관 | [`np.corrcoef(...)`](https://github.com/gmrmsy/CLAP_D/blob/main/scr/evaluate/test.py#L70) | 실제 임상적 유용성을 더 잘 반영 |
...
1. **Accuracy는 불균형 데이터에서 잘못된 신호를 줄 수 있다** — 상관계수·F1·AUC 등의 보조 지표를 항상 함께 확인해야 한다.
2. **모델 복잡도 이전에 데이터 품질이 우선이다** — 구조를 아무리 정교하게 해도 근본적인 점수 분포 편향과 데이터 부족이 해결되지 않으면 한계가 있다.
3. **출력층 설계는 손실 함수와 함께 검토해야 한다** — relu + MSE 조합처럼 수학적으로 충돌하는 조합이 있다.
4. **증강은 만능이 아니다** — 피치·속도 변환 증강은 특정 문항에서 corr을 높이는 효과가 있었지만, 전체 예측 성능을 안정적으로 개선하지는 못했다.
...
│   ├── models/
│   │   ├── model_1D.py            # 1D CNN 기반 Cross-Attention 모델
│   │   └── model_2D.py            # 2D CNN 기반 Cross-Attention 모델
│   ├── utils/
...
Conv1D/2D · Bidirectional GRU · Multi-Head Attention · Sinusoidal Positional Encoding  
멜 스펙트로그램 · 한국어 자모 분리 · 음성 증강 (피치·속도)
